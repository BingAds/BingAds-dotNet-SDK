    ...
    public abstract class ExampleBase
    {
        ...
        protected void OutputCampaign(Campaign campaign)
        {
            if (campaign != null)
            {
                ...
                OutputStatusMessage(string.Format("ConversionTrackingEnabled: {0}", campaign.ConversionTrackingEnabled));
                OutputStatusMessage(string.Format("DailyBudget: {0}", campaign.DailyBudget));
                OutputStatusMessage(string.Format("DaylightSaving: {0}", campaign.DaylightSaving));
        ...
        protected void OutputProductAd(ProductAd ad)
        {
            if (ad != null)
            {
                ...
                OutputStatusMessage(string.Format("Id: {0}", ad.Id));
                OutputStatusMessage(string.Format("PhoneNumber: {0}", ad.PromotionalText));
                OutputStatusMessage(string.Format("Status: {0}", ad.Status));
            }
        }
