...
namespace BingAdsExamplesLibrary_CORE.Version13
{
    public class CampaignManagementExampleHelper
    {
        public LogggingBase.SendMessageDelegate OutputStatusMessage;

        public ServiceClient<ICampaignManagementService> CampaignManagementService;
        public CampaignManagementExampleHelper(ServiceClient<ICampaignManagementService> _serviceClient)
        {
            OutputStatusMessage = LogggingBase.OutputStatusMessageDefault;
            CampaignManagementService = _serviceClient;
        }
        ...
        public void OutputMatchType(Microsoft.BingAds.V13.CampaignManagement.MatchType valueSet)
        {
            OutputStatusMessage(string.Format("Values in {0}", valueSet.GetType()));
            foreach (var value in Enum.GetValues(typeof(Microsoft.BingAds.V13.CampaignManagement.MatchType)))
            {
                OutputStatusMessage(value.ToString());
            }
        }
        public void OutputArrayOfMatchType(IList<Microsoft.BingAds.V13.CampaignManagement.MatchType> valueSets)
        {
            if (null != valueSets)
            {
                foreach (var valueSet in valueSets)
