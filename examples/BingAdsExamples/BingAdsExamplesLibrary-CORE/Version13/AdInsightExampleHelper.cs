...
namespace BingAdsExamplesLibrary_CORE.Version13
{
    public class AdInsightExampleHelper
    {
        ...
        public async Task<GetEstimatedPositionByKeywordsResponse> GetEstimatedPositionByKeywordsAsync(
            ...
            IList<long> locationIds,
            CurrencyCode? currencyCode,
            IList<Microsoft.BingAds.V13.AdInsight.MatchType> matchTypes,
            long? campaignId,
            long? adGroupId)
        {
        ...
        public void OutputMatchType(Microsoft.BingAds.V13.AdInsight.MatchType valueSet)
        {
            OutputStatusMessage(string.Format("Values in {0}", valueSet.GetType()));
            foreach (var value in Enum.GetValues(typeof(Microsoft.BingAds.V13.AdInsight.MatchType)))
            {
                OutputStatusMessage(value.ToString());
            }
        }
        public void OutputArrayOfMatchType(IList<Microsoft.BingAds.V13.AdInsight.MatchType> valueSets)
        {
            if (null != valueSets)
            {
                foreach (var valueSet in valueSets)
