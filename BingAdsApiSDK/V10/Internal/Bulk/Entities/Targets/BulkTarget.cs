...
namespace Microsoft.BingAds.V10.Internal.Bulk.Entities
{
    ...
    public abstract class BulkTarget<TIdentifier, TAgeBid, TAge, TGenderBid, TGender, TDayTimeBid, TDayTime,
        TLocationBid, TLocation, TNegativeLocationBid, TNegativeLocation, TRadiusTargetBid, TRadius, TDeviceOsBid,
        TDeviceOs> : MultiRecordBulkEntity
        where TIdentifier : BulkTargetIdentifier
        where TAgeBid : BulkAgeTargetBid
        where TAge : BulkAgeTarget<TAgeBid>
        where TGenderBid : BulkGenderTargetBid
        where TGender : BulkGenderTarget<TGenderBid>
        where TDayTimeBid : BulkDayTimeTargetBid
        where TDayTime : BulkDayTimeTarget<TDayTimeBid>
        where TLocationBid : BulkLocationTargetBid
        where TLocation : BulkLocationTarget<TLocationBid>
        where TNegativeLocationBid : BulkNegativeLocationTargetBid
        where TNegativeLocation : BulkNegativeLocationTarget<TNegativeLocationBid>
        where TRadiusTargetBid : BulkRadiusTargetBid
        where TRadius : BulkRadiusTarget<TRadiusTargetBid>
        where TDeviceOsBid : BulkDeviceOsTargetBid
        where TDeviceOs : BulkDeviceOsTarget<TDeviceOsBid>
    {
        ...
        protected BulkTarget(BulkTargetBid bid, TLocation location, TAge age, TGender gender, TDayTime dayTime,
            TDeviceOs deviceOs, TNegativeLocation negativeLocation, TRadius radius)
            : this((TIdentifier) bid.Identifier, location, age, gender, dayTime, deviceOs, negativeLocation, radius)
        {
            _bids.Add(bid);
        }

        internal BulkTarget(TIdentifier identifier, TLocation location, TAge age, TGender gender, TDayTime dayTime,
            TDeviceOs deviceOs, TNegativeLocation negativeLocation, TRadius radius)
        ...
        /// <param name="negativeLocation">Reserved for internal use.</param>
        /// <param name="radius">Reserved for internal use.</param>
        protected BulkTarget(TLocation location, TAge age, TGender gender, TDayTime dayTime, TDeviceOs deviceOs,
            TNegativeLocation negativeLocation, TRadius radius)
        {
        ...
        private readonly List<BulkTargetBid> _bids = new List<BulkTargetBid>();

        private readonly List<TIdentifier> _deleteAllRows = new List<TIdentifier>();
            ...
            where T : BulkTargetBid
        {
            ...
            {
                target.Status = V10.Bulk.Entities.Status.Deleted;
                return;
            }
        ...
        private void PopulateChildTargetIdentities<T>(BulkSubTarget<T> target,
            Dictionary<Type, List<TIdentifier>> groups)
            where T : BulkTargetBid
        {
